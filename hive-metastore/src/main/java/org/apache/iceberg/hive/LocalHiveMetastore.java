...
public class LocalHiveMetastore {
  ...
  private static final Logger LOG = LoggerFactory.getLogger(LocalHiveMetastore.class);
  ...
  LocalHiveMetastore(String userHiveLocalDir) {
    try {
      if (userHiveLocalDir == null) {
        hiveLocalDir =
            createTempDirectory("hive", asFileAttribute(fromString("rwxrwxrwx"))).toFile();
      } else {
        hiveLocalDir = new File(userHiveLocalDir);
      }
      File metastoreDb = new File(hiveLocalDir, "metastore_db");
      derbyPath = metastoreDb.getPath();
  ...
  /**
   * Starts a LocalHiveMetastore with the default connection pool size (5) and the default HiveConf.
   */
  public void start() {
  ...
  /**
   * Starts a LocalHiveMetastore with the default connection pool size (5) with the provided
   * HiveConf.
   *
  ...
  /**
   * Starts a LocalHiveMetastore with a provided connection pool size and HiveConf.
   *
   * @param conf The hive configuration to use
