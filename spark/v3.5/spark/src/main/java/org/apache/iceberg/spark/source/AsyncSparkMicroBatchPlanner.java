...
import static org.apache.iceberg.spark.source.SparkMicroBatchStream.computeInitialOffset;
import static org.apache.iceberg.spark.source.StreamingOffset.START_OFFSET;
...
public class AsyncSparkMicroBatchPlanner implements SparkMicroBatchPlanner {
    ...
    /**
     * This class manages a queue of FileScanTask + StreamingOffset.
     * On creation, it starts up an asynchronous polling process which populates it to the queue when a new snapshot
     * arrives or the minimum amount of queued data is too low
     * <p>
     * Note: that this will in effect capture the state of the table when snapshots are added to the queue.  If a snapshot
     * is expired after it has been added to the queue, the job will still process it.
     */
    public AsyncSparkMicroBatchPlanner(
        ...
        this.planFilesCache = CacheBuilder.newBuilder()
                .maximumSize(10)
                .build();
        ...
        LOG.info("Started AsyncSparkMicroBatchPlanner for {} from initialOffset: {}", table.name(), initialOffset);
    }
    ...
    public synchronized void stop() {
        ...
        try {
            stopped = executor.awaitTermination(readConf.streamingSnapshotPollingIntervalMs() * 2, TimeUnit.MILLISECONDS);
     ...
     */
    @Override
    public synchronized List<FileScanTask> planFiles(StreamingOffset startOffset, StreamingOffset endOffset) throws ExecutionException {
        return planFilesCache.get(
                Pair.of(startOffset, endOffset),
                () -> {
                    LOG.info("running planFiles for {}, startOffset: {}, endOffset: {}", table.name(), startOffset, endOffset);
                    ...
                    do {
                        ...
                        if (elem != null) {
                            ...
                            boolean endOffsetSynthetic = currentOffset.snapshotId() == endOffset.snapshotId()
                    ...
                    if (refreshFailedThrowable != null) throw new ExecutionException(refreshFailedThrowable);
                    LOG.info(
                            "completed planFiles for {}, startOffset: {}, endOffset: {}, files: {}, rows: {}",
                            table.name(), startOffset, endOffset, filesInPlan, rowsInPlan
                    );
                    return result;
                }
        );
    ...
    /**
     * This needs to be non destructive on the queue as spark could call this multiple times.
     * Each time, depending on the table state it could return something different
     *
    ...
    public synchronized StreamingOffset latestOffset(StreamingOffset startOffset, ReadLimit limit) {
        LOG.info("running latestOffset for {}, startOffset: {}, limit: {}", table.name(), startOffset, limit);
        ...
        if (refreshFailedThrowable != null) throw new RuntimeException(refreshFailedThrowable);
        if (fillQueueFailedThrowable != null) throw new RuntimeException(fillQueueFailedThrowable);
        ...
        SparkMicroBatchStream.UnpackedLimits unpackedLimits = new SparkMicroBatchStream.UnpackedLimits(limit);
        long rowsSeen = 0;
        long filesSeen = 0;
        LOG.debug("latestOffset queue status, queuedFiles: {}, queuedRows: {}", queuedFileCount.get(), queuedRowCount.get());
        for (Pair<StreamingOffset, FileScanTask> elem : queue) {
            ...
            if ((rowsSeen + fileRows > unpackedLimits.getMaxRows()) || (filesSeen + 1 > unpackedLimits.getMaxFiles())) {
        ...
        if (tail != null) {
            ...
            // we have to increment the position by 1 since we want to include the tail in the read and position is non-inclusive
            StreamingOffset latestOffset = new StreamingOffset(
    ...
    public Offset reportLatestOffset() {
        // intentionally not calling table.refresh() here so the background thread is the only thing making that call
        Snapshot latestSnapshot = table.currentSnapshot();
        StreamingOffset latestOffset = null;
        if (latestSnapshot != null) {
            latestOffset = new StreamingOffset(
    ...
    private void addMicroBatchToQueue(
            ...
            long startFileIndex,
            long endFileIndex,
            boolean shouldScanAllFile
    ) {
        LOG.info("Adding MicroBatch for snapshot: {} to the queue", snapshot.snapshotId());
        MicroBatches.MicroBatch microBatch = MicroBatches
                .from(snapshot, table.io())
        ...
        for (FileScanTask task : microBatch.tasks()) {
            Pair<StreamingOffset, FileScanTask> elem = Pair.of(
                    new StreamingOffset(
...
//            fillQueue(lastQueuedSnapshot);
    ...
    private void fillQueue(Snapshot readFrom) {
        if ((queuedRowCount.get() > minQueuedRows) || (queuedFileCount.get() > minQueuedFiles)) {
            ...
            LOG.debug(
                    "Buffer is full, {} > {} or {} > {}",
                    queuedRowCount.get(), minQueuedRows, queuedFileCount.get(), minQueuedFiles
            );
        } else {
            ...
            if (nextValidSnapshot != null) {
                addMicroBatchToQueue(
                        nextValidSnapshot,
                        0,
                        addedFilesCount(nextValidSnapshot),
                        false
                );
            } else {
    ...
    private Snapshot nextValidSnapshot(Snapshot curSnapshot) {
        ...
        if (curSnapshot == null) {
            StreamingOffset startingOffset = computeInitialOffset(table, readConf.streamFromTimestamp());
            LOG.debug("computeInitialOffset picked startingOffset: {}", startingOffset);
            if (START_OFFSET.equals(startingOffset)) {
                return null;
            }
