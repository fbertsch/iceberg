...
public class SyncSparkMicroBatchPlanner implements SparkMicroBatchPlanner {
  ...
  public List<FileScanTask> planFiles(StreamingOffset startOffset, StreamingOffset endOffset) {
    ...
    StreamingOffset batchStartOffset =
            StreamingOffset.START_OFFSET.equals(startOffset)
                    ? computeInitialOffset(table, fromTimestamp)
                    : startOffset;
    ...
    do {
      ...
      if (currentOffset == null) {
        ...
        if (currentOffset.snapshotId() != endOffset.snapshotId()) {
          currentOffset = new StreamingOffset(
                  snapshotAfter.snapshotId(),
                  0L,
                  false,
                  snapshotAfter.timestampMillis(),
                  Long.parseLong(snapshotAfter.summary().getOrDefault(TOTAL_RECORDS, "-1"))
          );
        } else {
  ...
  public StreamingOffset latestOffset(StreamingOffset startOffset, ReadLimit limit) {
    ...
    if (startOffset.equals(StreamingOffset.START_OFFSET)) {
      startingOffset = computeInitialOffset(table, fromTimestamp);
    }
    ...
    StreamingOffset latestStreamingOffset =
            new StreamingOffset(
                    ...
                    Long.parseLong(curSnapshot.summary().getOrDefault(TOTAL_RECORDS, "-1"))
            );
