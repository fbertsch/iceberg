...
abstract class BaseScan implements Scan, Batch, SupportsReportStatistics {
  ...
  private final InMemoryFileIndex fileIndex;
  ...
  public InputPartition[] planInputPartitions() {
    ...
    List<PartitionedFile> files = partitions.stream()
        .flatMap(part -> JavaConverters.seqAsJavaListConverter(part.files()).asJava().stream().map(
            stat -> PartitionedFile.apply(part.values(),
                    SparkPath.fromPath(stat.getPath()), 0, stat.getLen(), NO_LOCALITY, 0, 0,
                    ScalaUtil.asScala(emptyMap()))))
  ...
  private Stream<PartitionedFile> split(PartitionedFile file, long splitSize) {
    ...
    while (remainingLen > 0) {
      ...
      splits.add(PartitionedFile.apply(file.partitionValues(), file.filePath(), offset, len, file.locations(),
              0, 0, ScalaUtil.asScala(emptyMap())));
      offset += len;
      remainingLen -= len;
    }
  ...
  public String description() {
    String filters = partFilters.stream().map(Spark3Util::describe).collect(Collectors.joining(", "));
  ...
  public boolean equals(Object o) {
    ...
    BaseScan that = (BaseScan) o;
    return table.name().equals(that.table.name()) &&
        expectedSchema.equals(that.expectedSchema) &&
        partFilters.toString().equals(that.partFilters.toString()) &&
        options.equals(that.options);
  }

  @Override
  public int hashCode() {
    return Objects.hash(table.name(), expectedSchema, partFilters.toString(), options);
  }
  ...
  abstract static class TaskReader<T> implements PartitionReader<T> {
    ...
    private Iterator<InternalRow> openFile(PartitionedFile file) {
      InputFileBlockHolder.set(file.filePath().urlEncoded(), file.start(), file.length());
      return open(file);
    }
  ...
  private static InMemoryFileIndex buildFileIndex(SparkSession spark, MetacatSparkTable table,
                                                        List<PartitionDto> partitions) {
  ...
  static InternalRow partitionTuple(org.apache.iceberg.PartitionSpec spec, PartitionDto part) {
    ...
    PartitionKey key = new PartitionKey(spec, spec.schema());
