...
import com.netflix.iceberg.metacat.CommonViewCatalog;
...
public class NetflixSparkCatalog extends BaseCatalog implements ViewCatalog {
  ...
  public void replaceView(
    ...
    if(viewCatalog instanceof CommonViewCatalog) {
      ...
      // Create new array to hold catalog in first position
      String[] catalogAndNamespace = new String[currentNamespace.length + 1];
      catalogAndNamespace[0] = currentCatalog;
      System.arraycopy(currentNamespace, 0, catalogAndNamespace, 1, catalogAndNamespace.length);

      ((CommonViewCatalog) viewCatalog).replaceView(ident, sql, schema, catalogAndNamespace, properties);
    } else {
