...
public class StreamingOffset extends Offset {
   ...
   * @param scanAllFiles whether to scan all files in a snapshot; for example, to read all data when starting a stream.
   * @param snapshotTimestampMillis The time the snapshot was created
   * @param snapshotTotalRows Total rows in the snapshot
   */
  public StreamingOffset(long snapshotId, long position, boolean scanAllFiles, long snapshotTimestampMillis, long snapshotTotalRows) {
  ...
  @Override
  public int hashCode() {
    return Objects.hashCode(snapshotId, position, scanAllFiles, snapshotTimestampMillis, snapshotTotalRows);
  }
  ...
  private static StreamingOffset fromJsonNode(JsonNode node) {
    ...
    long snapshotTimestampMs = Optional.ofNullable(JsonUtil.getLongOrNull(SNAPSHOT_TIMESTAMP_MS, node)).orElse(-1L);
    long snapshotTotalRows = Optional.ofNullable(JsonUtil.getLongOrNull(SNAPSHOT_TOTAL_ROWS, node)).orElse(-1L);

    return new StreamingOffset(snapshotId, position, shouldScanAllFiles, snapshotTimestampMs, snapshotTotalRows);
  }
