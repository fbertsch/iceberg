...
import static org.apache.iceberg.metrics.CommitMetricsResult.TOTAL_RECORDS;

public class SparkMicroBatchStream implements MicroBatchStream, SupportsAdmissionControl, ReportsSourceMetrics {
  ...
  public InputPartition[] planInputPartitions(Offset start, Offset end) {
    ...
    if (microBatchPlanner == null) {
      if (readConf.asyncMicroBatchPlanningEnabled()) {
        ...
        this.microBatchPlanner = new AsyncSparkMicroBatchPlanner(table, readConf, startOffset, endOffset);
      } else {
        this.microBatchPlanner = new SyncSparkMicroBatchPlanner(table, readConf);
      }
    }
  ...
  public Offset latestOffset(Offset startOffset, ReadLimit limit) {
    if (microBatchPlanner == null) {
      if (readConf.asyncMicroBatchPlanningEnabled()) {
        // this will block while the planner fills the queue
        this.microBatchPlanner = new AsyncSparkMicroBatchPlanner(table, readConf, (StreamingOffset) startOffset, null);
      } else {
        this.microBatchPlanner = new SyncSparkMicroBatchPlanner(table, readConf);
      }
    }
    LOG.debug("latestOffset, from: {}, limit: {}", startOffset.json(), limit.toString());
    ...
    StreamingOffset latestOffset = microBatchPlanner.latestOffset((StreamingOffset) startOffset, limit);
  ...
  public String toString() {
    return SparkMicroBatchStream.class.getCanonicalName();
  }
  ...
  public static StreamingOffset computeInitialOffset(Table table, long fromTimestamp) {
    ...
    if (fromTimestamp == Long.MIN_VALUE) {
      LOG.debug("computeInitialOffset: fromTimestamp is Long.MIN_VALUE, returning oldest snapshot");
      ...
      return new StreamingOffset(
              ...
              Long.parseLong(oldestSnapshot.summary().getOrDefault(TOTAL_RECORDS, "-1"))
      );
    }
    ...
    try {
      ...
      if (snapshot != null) {
        return new StreamingOffset(
                ...
                Long.parseLong(snapshot.summary().getOrDefault(TOTAL_RECORDS, "-1"))
        );
      } else {
  ...
  public Map<String, String> metrics(Optional<Offset> latestConsumedOffset) {
    ...
    metricsMap.put("details",  writer.toString());
  ...
  public static class UnpackedLimits {
    ...
    public UnpackedLimits(ReadLimit limit) {
      if (limit instanceof CompositeReadLimit) {
        ...
        Arrays.stream(compositeLimits).forEach((individualLimit) -> {
