...
class BatchUtil {
  ...
  private static <T> String sqlString(List<Literal<T>> lits) {
    return lits.stream().map(BatchUtil::sqlString).collect(Collectors.joining(", "));
  }
  ...
  static boolean equalsIgnoreCaseAndNullability(DataType from, DataType to) {
    if (from instanceof ArrayType && to instanceof ArrayType) {
      return equalsIgnoreCaseAndNullability(((ArrayType) from).elementType(), ((ArrayType) to).elementType());
    } else if (from instanceof MapType && to instanceof MapType) {
      return equalsIgnoreCaseAndNullability(((MapType) from).keyType(), ((MapType) to).keyType()) &&
          equalsIgnoreCaseAndNullability(((MapType) from).valueType(), ((MapType) to).valueType());
    } else if (from instanceof StructType && to instanceof StructType) {
      StructType fromStruct = (StructType) from;
      StructType toStruct = (StructType) to;
      if (fromStruct.length() != toStruct.length()) {
        return false;
      }
      for (int i = 0; i < fromStruct.length(); ++i) {
        StructField l = fromStruct.fields()[i];
        StructField r = toStruct.fields()[i];
        if (!l.name().equalsIgnoreCase(r.name()) ||
