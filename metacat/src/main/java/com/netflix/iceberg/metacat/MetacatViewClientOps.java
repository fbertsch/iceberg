...
class MetacatViewClientOps extends BaseMetastoreViewOperations {
    ...
    public synchronized ViewVersionMetadata refresh() {
        ...
        try {
            ...
            if (!"true".equalsIgnoreCase(common_view_flag)) {
                throw new NotFoundException(
                    "Entity %s.%s.%s exists but not a View, common_view flag: %s",
                    catalog, dbName, viewName, common_view_flag);
            }

            metadataLocation = tableProperties.get(BaseMetastoreViewOperations.METADATA_LOCATION_PROP);
    ...
    public synchronized void commit(ViewVersionMetadata base, ViewVersionMetadata metadata, Map<String, String> properties) {
        ...
        try {
            ...
            if (metadata.properties().containsKey("owner")) {
                ObjectNode definitionMetadata = JsonNodeFactory.instance.objectNode();
                definitionMetadata.put("owner",
                        JsonNodeFactory.instance.objectNode().put("userId", metadata.properties().get("owner")));
            ...
            metadata_props.put(BaseMetastoreViewOperations.METADATA_LOCATION_PROP, newMetadataLocation);
            metadata_props.put(BaseMetastoreViewOperations.PREVIOUS_METADATA_LOCATION_PROP, currentMetadataLocation() == null ? "" : currentMetadataLocation());
    ...
    public static String getMetacatTypeName(Type type) {
        switch (type.typeId()) {
            ...
            case DECIMAL:
                ...
                return String.format("decimal(%s,%s)", decimalType.precision(), decimalType.scale()); //TODO may be just decimal?
            case STRUCT:
                final Types.StructType structType = type.asStructType();
                final String nameToType = structType.fields().stream().map(
                        f -> String.format("%s:%s", f.name(), getMetacatTypeName(f.type()))
