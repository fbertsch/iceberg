...
public class DefinitionMetadata {
  ...
  public static String getOwnerUserId(ObjectNode definitionMetadata) {
    if (definitionMetadata != null && definitionMetadata.hasNonNull(MetacatUtil.OWNER)) {
      JsonNode definitionMetadataOwner = definitionMetadata.get(MetacatUtil.OWNER);
      return definitionMetadataOwner.get(MetacatUtil.USER_ID).asText("");
    }
    return "";
  }
